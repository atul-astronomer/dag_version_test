...
def get_logs_chunks(log_id):
    ...
    es = Elasticsearch(['http://{}:{}'.format(str(url_components[4]), str(url_components[5]))],
    ...
    resp = es.search(index="fluentd.{}.*".format(str(url_components[2])),
                     body={"_source": ["message"], "query": query, "size": 10000}, scroll='10m')
...
def log_checker(ti_id: str, expected: str, notexpected: str, try_number: int = 1, **context: dict):
    ...
    def check(ti, expect, notexpect):
        ...
        if remote_logging == True and logging_store_uri == "":
