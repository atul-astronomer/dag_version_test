...
def rand_graph(node_ct, edge_probability, min_islands, rand_seed):
    ...
    while remaining_node_ct > 0:
        ...
        while edge_ct == 0:
            print(
                f"generating {this_pass_node_ct} nodes with edge_probability {edge_probability} and seed {rand_seed}"
            )
        ...
        if prefix_offset + isl_ct > 26:
            raise Exception(
                f"got {len(islands)} islands, can't use letters to index them"
            )
...
def get_dags(g: DiGraph):
    ...
    return DatasetNetwork(
        datasets=datasets, dag_families=dag_families, longest_path=longest_path
    )
...
def gen_dags(dataset_network: DatasetNetwork):
    ...
    def dag_def(dag: Dag, file_datasets):
        ...
        if dag.inlet_datasets:
            ...
            kwargs += f"schedule_interval=None,"
