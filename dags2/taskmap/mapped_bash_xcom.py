...
with DAG(
    ...
    dynamic = BashOperator.partial(task_id="bash", do_xcom_push=True).expand(
        env=envs(), bash_command=cmds()
    )

    s = DummyOperator(task_id="static_start")
    d = DummyOperator(task_id="static_done")

    static_ids = []
    for i, (e, c) in enumerate(product([foo_var, bar_var], [hi_cmd, bye_cmd])):
        task = BashOperator(
            task_id=f"bash{i + 1}", env=e, bash_command=c, do_xcom_push=True
        )
        (s >> task >> d)
