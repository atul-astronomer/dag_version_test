...
with DAG(
    ...
    pick() >> TriggerDagRunOperator(
        task_id="trigger_taskgroups", trigger_dag_id="fake_dyn_taskgroups"
    )
...
with DAG(
    ...
    for tg, tasks in Variable.get(
        "structure", deserialize_json=True, default_var=scenes[-1]
    ).items():
        with TaskGroup(tg):
