...
from airflow.decorators import setup, task, teardown, task_group
...
with DAG(
    dag_id="example_setup_one_to_many_as_teardown", start_date=datetime(2022, 3, 4), catchup=False,tags=["setup_teardown_neg"]
) as dag:
    """-- passing
...
with DAG(
    dag_id="example_setup_one_to_many_as_teardown_offd", start_date=datetime(2022, 3, 4), catchup=False,tags=["setup_teardown"]
) as dag:
    """-- passing
    1 setup mapping to 3 teardowns
    1 work task
...
with DAG(dag_id="example_mapped_task_group_simple", start_date=datetime(2022, 3, 4), catchup=False,tags=["setup_teardown_neg"]) as dag:
    """-- passing
...
with DAG(
    dag_id="example_mapped_task_group_work_fail_or_skip", start_date=datetime(2022, 3, 4), catchup=False,tags=["setup_teardown_neg"]
) as dag:
    """-- passing
    Mapped task group wherein there's a simple s >> w >> t pipeline.
...
with DAG(
    dag_id="example_setup_teardown_many_one_explicit", start_date=datetime(2022, 3, 4), catchup=False,tags=["setup_teardown_neg"]
) as dag:
    """-- passing
...
with DAG(dag_id="example_setup_one_to_many", start_date=datetime(2022, 3, 4), catchup=False,tags=["setup_teardown_neg"]) as dag:
