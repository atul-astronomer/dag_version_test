...
from dags.plugins.airflow_dag_introspection import assert_homomorphic
from dags.plugins.api_utility import get_task_instances
...
with DAG(
    ...
    def dec_task_grouper():
        ...
        def dummy8(val, **context):
            ...
            three_upstream = list(
                filter(lambda ti: "group2.dummy5" in ti.get('task_id'), task_instances)
            )
            print("three_upstream: ", three_upstream)
            two_upstream = list(
                filter(lambda ti: "group2.dummy6" in ti.get('task_id'), task_instances)
            )
            print("two_upstream: ", two_upstream)
            one_upstream = list(
                filter(lambda ti: "group2.dummy7" in ti.get('task_id'), task_instances)
            )
