...
dag = DAG(
    ...
    tags=["core", "extended_dags" "k8","k8s-pod"],
)
...
def get_pod_specs(regex_list,ti):
    ...
    for regex in regex_list:
        ...
        pod_specs = subprocess.run(['kubectl', 'get', 'pod', get_pod_name, '-o', 'json','-n',namespace], capture_output=True, text=True).stdout
        # Store the pod name in an XCom variable
...
k8s_resource_requirements_negative_1 = k8s.V1ResourceRequirements(
...
k8s_resource_requirements_negative_2 = k8s.V1ResourceRequirements(
        limits={"memory": "256Mi"}
    )
...
k8s_resource_requirements_negative_3 = k8s.V1ResourceRequirements(
        limits={"cpu": "100m"}
    )
...
k8s_resource_requirements_negative_4 = k8s.V1ResourceRequirements(
...
k8s_resource_requirements_negative_5 = k8s.V1ResourceRequirements(
        limits={"memory": "2Gi"}
    )
...
k8s_resource_requirements_negative_6 = k8s.V1ResourceRequirements(
        limits={"cpu": 1}
    )
...
verify_kpo_pod_specs_task = PythonOperator(
    ...
    op_args=[['limits-only-with-empty-cpu','limits-only-with-empty-mem','requests-only-with-empty-cpu','requests-only-with-empty-mem']],
    dag=dag
)
