...
from dags.datasets.setup_planets import conn_id, csv_path
...
def announce_champion():
    ...
    result = pg_hook.get_records(
        sql="SELECT planet FROM planetary_mass_rank WHERE rank = '1';"
    )
...
with DAG(
    ...
    PostgresOperator(
        task_id="insert_rows", sql=build_sql(), postgres_conn_id=conn_id, outlets=[mass]
    )
...
with DAG(
    ...
    def announce_champion():
        pg_hook = PostgresHook(postgres_conn_id=conn_id)
        result = pg_hook.get_records(
            sql=f"SELECT planet FROM planetary_mass_rank WHERE rank = 1;"
        )
        print(result[0], "is the most massive planet")
